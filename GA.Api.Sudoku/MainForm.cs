...
namespace GA.Api.Sudoku
{
    public partial class MainForm : Form
    {
        ...
        {
            grid.Set(data);            
            gs = new GeneticSolver(5000, 10000, 0.8, 0.35, CrossoverType.UniformCrossover);
            gs.GeneratorFunction = VariablesGenerator2;
        ...
        {            
            ...
            var empty_values = data.GetEmptyValues();

            foreach (var empty_value in empty_values)
            {
                rnd = new Random(Guid.NewGuid().GetHashCode());
                variables.Add(rnd.Next(1, 10));
            }

            return new Chromosome(variables);
        }

        public Chromosome VariablesGenerator2()
        {
            var variables = new List<object>();
        ...
        private void G_IterationCompleted(object? sender, Events.IterationCompletedEventArgs e)
        {
            ...
            lblAverageFitness.Text = $"Avg : {e.AverageFitness} Best {e.BestChromosome.Fitness}";
